...
def main():
    ...
    try:
        ...
        for c in containers:
            print(f"{c}:")
            # Check if Tirith binary exists
            status, out, err = run_command(ssh, f"docker exec {c} which tirith")
            if status == 0:
                print(f"  [OK] Tirith found at: {out.strip()}")
                
                # Add hook to .bashrc
                cmd = f"docker exec {c} bash -c 'grep -q \"tirith init\" ~/.bashrc || echo \"eval \\\"\\$(tirith init --shell bash)\\\"\" >> ~/.bashrc'"
